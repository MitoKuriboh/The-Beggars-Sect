...
export const CombatAnimation: React.FC<CombatAnimationProps> = ({ config }) => {
  ...
  const getFrames = (): string[] => {
    switch (config.type) {
      ...
      case "technique":
        return TECHNIQUE_FRAMES;
      case "defend":
        return DEFEND_FRAMES;
      case "chi":
        return CHI_FRAMES;
      case "impact":
...
export const DirectionalAttack: React.FC<DirectionalAttackProps> = ({
  ...
  const [phase, setPhase] = useState<"windup" | "strike" | "impact" | "done">(
    "windup",
  );
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- frames are stable, only animate on frameIndex/phase
