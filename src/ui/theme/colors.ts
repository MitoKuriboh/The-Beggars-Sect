...
export function getSpeakerColor(speaker: string): InkColor {
  ...
  if (upper.includes('OLD DAO') || upper.includes('DAO') || upper.includes('ELDER')) {
    return narrativeColors.elder;
  }

  // Enemies/Hostile
  if (
    upper.includes('THUG') || upper.includes('PUNK') ||
    upper.includes('BRAWLER') || upper.includes('SPARTAN') ||
    upper.includes('RAZOR') || upper.includes('VEX') || upper.includes('HOLLOW')
  ) {
    return narrativeColors.enemy;
  }

  // Beggars Sect members
  if (upper.includes('BEGGAR') || upper.includes('RAT') || upper.includes('FENG')) {
