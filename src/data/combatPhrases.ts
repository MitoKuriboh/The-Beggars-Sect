...
function getDamageTier(
  damage: number,
  targetMaxHp: number,
): keyof typeof DAMAGE_PHRASES {
  // Guard against division by zero
  if (targetMaxHp <= 0) return "light";
...
export function getTechniqueMessage(
  ...
  const phrases =
    TECHNIQUE_PHRASES[techniqueId] ?? TECHNIQUE_PHRASES.default ?? [];
 ...
 */
export function getStanceMessage(
  actorName: string,
  stanceName: string,
): string {
  const phrases = STANCE_PHRASES[stanceName] ?? [
    `{actor} changes to ${stanceName}`,
  ];
  const phrase = getRandomPhrase(phrases);
